...
>
  Delete
            ...
            <div className="kpis">
              <KpiCard
                label="Revenue / mo"
                value={aud0.format((scenario as any).inputs.salePricePerUnit * (scenario as any).inputs.monthlyDemand)}
              />
              <KpiCard
                label="Margin %"
              ...
              <KpiCard
                ...
                tone={alerts.belowMin > 0 ? 'bad' : alerts.reorder > 0 ? 'warn' : 'good'}
              />
            </div>
              ...
              <div className="layout-grid">
                <div className="card">
                  ...
                  <div className="small">
                    ...
                    <br />• Schedule (next 7 days): <b>{sched.ready}</b> ready, <b>{sched.atRisk}</b> at risk,{' '}
                    <b>{sched.blocked}</b> blocked
                    <br />• Stock: <b>{alerts.belowMin}</b> below min, <b>{alerts.reorder}</b> reorder
                    <br />• Open issues: <b>{alerts.openIssues}</b> · Open CAPAs: <b>{alerts.openCapas}</b> · Machines down:{' '}
                    <b>{alerts.machinesDown}</b>
                  </div>

                  <div className="hint" style={{ marginTop: 12 }}>
                    Most value comes from: Resources → Production → Stock.
                    <br />
                    Drag a batch onto the calendar to auto-create the process chain, then assign people/machines and update
                    statuses.
                  </div>
                </div>
                ...
                <div className="card">
                  ...
                  <ul className="small">
                    {sched.atRisk > 0 && (
                      <li>
                        <b>Schedule at risk</b> — conflicts detected (double-booking or maintenance clash). Fix in
                        Production.
                      </li>
                    )}
                    {sched.blocked > 0 && (
                      <li>
                        <b>Blocked work</b> — items in Issue/Quarantine/Cancelled. Review and add CAPAs if needed.
                      </li>
                    )}
                    {alerts.belowMin > 0 && (
                      <li>
                        <b>Stock below minimum</b> — check Stock tab and raise orders.
                      </li>
                    )}
                    {alerts.reorder > 0 && (
                      <li>
                        <b>Stock at reorder</b> — review reorder points and lead times.
                      </li>
                    )}
                    {alerts.openCapas > 0 && (
                      <li>
                        <b>Open CAPAs</b> — assign owners and due dates in Decisions & Tracking.
                      </li>
                    )}
                    {alerts.machinesDown > 0 && (
                      <li>
                        <b>Machines out of service</b> — update Resources and schedule Maintenance blocks.
                      </li>
                    )}
                    {sched.atRisk === 0 &&
                      sched.blocked === 0 &&
                      ...
                      alerts.reorder === 0 &&
                      alerts.openCapas === 0 &&
                      alerts.machinesDown === 0 && <li>Nothing critical flagged right now.</li>}
                  </ul>
            ...
            {activeTab === 'Stock' && (
              <div>
                <div className="card">
                  ...
                  <div className="hint">
                    Remaining stock is calculated from <b>completed scheduled items</b>: Good units consume BOM;
                    post-assembly scrap consumes whole BOM; assembly scrap uses component rejects (or falls back to BOM if
                    empty).
                  </div>
                </div>
            ...
            {activeTab === 'Resources' && (
              <div>
                ...
                <EditableTable
                  ...
                  onChange={(rows) =>
                    updateScenario({ ...(scenario as any), machines: rows }, `Updated machines (${nowIso()})`)
                  }
                  createRow={() => ({
                    id: crypto.randomUUID(),
                    name: 'New machine',
                ...
                <EditableTable
                  ...
                  onChange={(rows) =>
                    updateScenario({ ...(scenario as any), processes: rows }, `Updated processes (${nowIso()})`)
                  }
                  createRow={() => ({
                    id: crypto.randomUUID(),
                    name: 'New process',
            ...
            {activeTab === 'Decisions & Tracking' && (
              <div>
                <EditableTable
                  ...
                  onChange={(rows) =>
                    updateScenario({ ...(scenario as any), decisions: rows }, `Updated decisions (${nowIso()})`)
                  }
                  createRow={() => ({
                    id: crypto.randomUUID(),
                    title: 'New decision',
            ...
            {activeTab === 'Warehouses' && (
              <EditableTable
                ...
                onChange={(rows) =>
                  updateScenario({ ...(scenario as any), warehouses: rows }, `Updated warehouses (${nowIso()})`)
                }
                createRow={() => ({
                  id: crypto.randomUUID(),
                  location: 'New DC',
